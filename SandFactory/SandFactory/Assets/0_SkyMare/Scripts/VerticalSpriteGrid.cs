...
public class VerticalSpriteGrid : MonoBehaviour
{
    ...
    public void Fit()
    {
        ...
        int rowsCount = Mathf.Clamp(rows, 0, count);
        if (count == 0 || rowsCount <= 0) return;

        // Số cột cần thiết (ceil)
        int cols = (count + rowsCount - 1) / rowsCount;

        // Kích thước toàn bộ vùng grid (tính theo world units)
        float fullWidth = cols * cellSize.x + Mathf.Max(0, cols - 1) * spacing.x;
        float fullHeight = rowsCount * cellSize.y + Mathf.Max(0, rowsCount - 1) * spacing.y;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // --- Tính vị trí theo hàng/cột (điền theo CỘT trước, từ trên xuống) ---
            int col = i / rowsCount;  // Thay đổi: chia cho rows để lấy cột
            int row = i % rowsCount;  // Thay đổi: modulo rows để lấy hàng
